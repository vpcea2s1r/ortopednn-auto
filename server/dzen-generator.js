...
async function callAI(prompt, opts = {}) {
  ...
  const resp = await fetch('https://api.openmodel.ai/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      system: opts.system || 'Ты — экспертный стоматолог-ортопед с 30-летним стажем. Пиши как для Дзена: живой язык, конкретика, примеры из практики. Ответь ТОЛЬКО JSON.',
